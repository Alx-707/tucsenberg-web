...
function parsePathnameForLink(currentPathname: string): LinkHref {
  ...
  // Check product category + slug pattern first (most specific)
  const categorySlugMatch = pathname.match(/^\/products\/([^/]+)\/([^/]+)$/);
  if (categorySlugMatch?.[1] && categorySlugMatch[2]) {
    return {
      pathname: "/products/[category]/[slug]",
      params: { category: categorySlugMatch[1], slug: categorySlugMatch[2] },
    };
  }

  // Check product category pattern
